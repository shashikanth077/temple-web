...
export function getDeceasedList(payload:any) {
    const baseUrl = `http://localhost:8080/api/deceasedlist/${payload.userid}`;
    const response = APICore.get(`${baseUrl}`, {});
    return response;
}

export function addDeasedRow(payload:any) {
    const baseUrl = `http://localhost:8080/api/deceased/${payload.userid}`;
    const response = APICore.create(`${baseUrl}`, payload);
    return response;
}

export function editDeasedRow(payload:any) {
    const baseUrl = `http://localhost:8080/api/deceased/${payload.userid}/${payload.id}`;
    const response = APICore.update(`${baseUrl}`, payload);
    return response;
}
