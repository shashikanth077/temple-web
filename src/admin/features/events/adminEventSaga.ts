...
import {
    addEvent, updateEvent, deleteEvent, getEventById, getEvents,
} from './adminEventApis';
...
import { SuccesResponse, EventSingleRes, EventListRes } from 'models';
...
function* fetcheventsList() {
    try {
        ...
        if (response.success) {
            yield put(adminEventActions.fetchEventListSuccess(response));
        } else {
            yield put(setError(response.errorMessage));
        }
    } catch (error) {
...
export function* watchfetcheventsList() {
    yield takeLatest(adminEventActions.fetchEvents.type, fetcheventsList);
}
...
function* adminEventSaga() {
    yield all([fork(watchfetcheventsList), fork(WatchgetEventById), fork(watchAddEvent), fork(watchupdateEvent), fork(watchdeleteEvent)]);
}
